...
const getUniqueSuffix = () => Math.random().toString(36).substring(2, 6);
...
test.describe('PIM Employee System Tests', { annotation: [{ type: 'Feature', description: 'PIM Module - Employee Management (UI & API/DB)' }] }, () => {
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      ...
      try {
        ...
        // 3. Click "Save"
        // #11: AddEmployeePage.save() has its own internal waitForResponse. Bypass it when using external InterceptorUtil.
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      // #9: Use short fixed strings for names, and add a unique suffix to avoid cross-test interference
      const uniqueSuffix = getUniqueSuffix();
      const employeeData = createEmployee({
        firstName: `Search-${uniqueSuffix}`,
        middleName: 'By',
        lastName: `Name-${uniqueSuffix}`,
      });
      let empNumber: number | undefined;
      ...
      try {
        ...
        // #12: Employee Name search is partial/substring match, not exact. Search by full name.
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      // #9: Use short fixed strings for names, and add unique suffixes to avoid cross-test interference
      const uniqueSuffix = getUniqueSuffix();
      // Use common term as prefix, but names themselves are unique.
      const commonPartialName = `Partial-${uniqueSuffix}`;
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      ...
      try {
        ...
        // #4: employeeListPage.searchButton is public, but this direct click is fine for ID search as no specific POM method exists.
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      // #12: Use a highly unique, non-matching string to avoid partial matches with existing data.
      const nonExistentName = `NonExistent_${getUniqueSuffix()}_XYZ`;
      ...
      // #6: getRecordCount() returns Promise, and already checks for 'Records (0) Found'
      expect(await employeeListPage.getRecordCount()).toBe(0);
      // Removed direct 'No Records Found' locator as getRecordCount is more robust.
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      // #9: Use short fixed strings for names, and add a unique suffix to avoid cross-test interference
      const uniqueSuffix = getUniqueSuffix();
      const employeeData = createEmployee({ firstName: `Detail-${uniqueSuffix}`, lastName: `View-${uniqueSuffix}` });
      let empNumber: number | undefined;
      ...
      try {
        ...
        await employeeListPage.navigate();
        // #12: Search by full name for better isolation when expecting specific single results.
        await employeeListPage.searchByName(`${employeeData.firstName} ${employeeData.lastName}`);
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      // #9: Use short fixed strings for names, and add a unique suffix to avoid cross-test interference
      const uniqueSuffix = getUniqueSuffix();
      const employeeData = createEmployee({ firstName: `DeleteMe-${uniqueSuffix}`, lastName: `Single-${uniqueSuffix}` });
      let empNumber: number | undefined;
      ...
      try {
        ...
        const deleteResponsePromise = authenticatedPage.waitForResponse('**/web/index.php/api/v2/pim/employees', { timeout: 10000 });
        ...
        // #12: Re-search by full name. If expecting 0 records, getRecordCount is more robust.
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      // #9: Use short fixed strings for names, and ensure uniqueness.
      // Use common term as prefix, but names themselves are unique.
      const uniqueSuffix = getUniqueSuffix();
      const commonSearchTerm = `MultiDelete-${uniqueSuffix}`; // Ensure uniqueness for cleanup
      ...
      try {
        ...
        // #12: Re-search by common partial name.
        await employeeListPage.searchByName(commonSearchTerm); // Re-search
        expect(await employeeListPage.getRecordCount()).toBe(0); // Should be zero
        await expect(employeeListPage.getByText('No Records Found')).toBeVisible();
  ...
  test(
    ...
    async ({ authenticatedPage }) => {
      // #9: Use short fixed strings for names, and add a unique suffix to avoid cross-test interference
      const uniqueSuffix = getUniqueSuffix();
      const employeeData = createEmployee({ firstName: `DoNotDelete-${uniqueSuffix}`, lastName: `Cancel-${uniqueSuffix}` });
      let empNumber: number | undefined;
