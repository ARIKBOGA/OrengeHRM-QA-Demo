...
test.describe('PIM Employee API CRUD', { annotation: [{ type: 'Feature', description: 'PIM Module - Employee Management (API)' }] }, () => {
  ...
  test(
    'CRUD-API-001: Create Employee (API) - Minimum Required Fields',
    {
      annotation: [
    ...
    async () => {
      ...
      try {
        ...
        // 4. Perform direct database query to confirm record exists
        ...
        // 5. Teardown: Delete the created employee via DB
        if (empNumber) {
  ...
  test(
    'CRUD-API-002: Create Employee (API) - All Fields',
    {
      annotation: [
    ...
    async () => {
      ...
      try {
        ...
        expect(response.data.lastName).toBe(employeeData.lastName);
        expect(response.data.employeeId).toBeDefined(); // OrangeHRM generates employeeId automatically, so just check it's present
  ...
  test(
    'CRUD-API-003: Create Employee (API) - Mandatory Field Violation',
    {
      annotation: [
    ...
    async ({ authenticatedApi }) => {
      ...
      if (config.dbEnabled) {
        // Conditional check for DB_ENABLED
  ...
  test(
    'CRUD-API-004: Update Employee (API) - Personal Details',
    {
      annotation: [
  ...
  test(
    'CRUD-API-005: Update Employee (API) - Mandatory Field Violation',
    {
      annotation: [
  ...
  test(
    'CRUD-API-006: Delete Employee (API) - Success',
    {
      annotation: [
  ...
  test(
    'CRUD-API-007: Delete Employee (API) - Non-existent Employee',
    {
      annotation: [
