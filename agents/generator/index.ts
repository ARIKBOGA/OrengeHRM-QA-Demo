...
function readContext(filename: string): string {
  ...
  return fs.existsSync(fullPath) ? fs.readFileSync(fullPath, 'utf-8') : `// CONTEXT FILE NOT FOUND: ${filename} — run npm run context:extract`;
}
...
async function main() {
  ...
  const sourceContext = {
    ...
    'src/pages/leave/assign-leave.page.ts': readSrc('src/pages/leave/assign-leave.page.ts'),
    'src/pages/leave/leave-list.page.ts': readSrc('src/pages/leave/leave-list.page.ts'),
    ...
    'src/utils/api/leave-type.util.ts': readSrc('src/utils/api/leave-type.util.ts'),
    ...
    'src/data/factories/leave.factory.ts': readSrc('src/data/factories/leave.factory.ts'),
  ...
  const raw = await gemini.generate(buildGeneratorPrompt(testPlanContent, sourceContext, groundTruth));
